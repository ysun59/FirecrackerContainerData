#For cat /proc/schedstat, use "schedstat_data_col_7.txt" to generate "schedstat_deltaData_col_7.txt"
# to generate "schedstat_fig_col_7.png"
#cpu0 0 0 0 0 0 0 2789857305742 120695662014 34103138 第七八九列数据，core 0~31
...
#取第N列的数据，横坐标：
#core0第N列，core1第N列，core2第N列，...core31第N列 <第一秒数据>
#core0第N列，core1第N列，core2第N列，...core31第N列 <第二秒数据>
#.....
#core0第N列，core1第N列，core2第N列，...core31第N列 <第60秒数据>
#go into folder by itself
folders = os.listdir('.')
def schedstatNoColTxt(n):
  for folder in folders:
    # schedstat
    cpu_perf_path = os.path.join('.', folder, 'schedstat.txt')
    if os.path.exists(cpu_perf_path):
      ...
      with open(cpu_perf_path, 'r') as file:
        ...
        for line in lines:
          ...
          # if line[0:3]=='cpu' or a[0:1] ==['version']:
          if line[0:3]=='cpu':
            # print('line is:')
            # print(line)
            if a[n:n+1]:
              ...
              if len(lineBuf)==0:
                # print('first time')
                lineBuf.append(''.join(a[n:n+1]))
                # print(lineBuf)
              else:
                lineBuf.append(','.join(a[n:n+1]))
                # print(lineBuf)

              # if a[0:1] == ['cpu3'] and len(lineBuf) != 0:
              # if a[0:1] == ['cpu3']:
              if a[0:1] == ['cpu31']:
      ...
      mpstat_csv_path = os.path.join('.', folder, 'schedstat_data_col_{}.txt'.format(n))
      with open(mpstat_csv_path, 'wt') as file:
        file.write('\n'.join(outputs))

schedstatNoColTxt(7) #取第N列数据生成txt，N从0开始
schedstatNoColTxt(8) 
schedstatNoColTxt(9) 


#calculate delta, eg: delta of core 0 at No_1s - No_0s
#core0第N列，core1第N列，core2第N列，...core31第N列 <第二秒数据 - 第一秒数据>
#core0第N列，core1第N列，core2第N列，...core31第N列 <第三秒数据 - 第二秒数据>
#.....
#core0第N列，core1第N列，core2第N列，...core31第N列 <第60秒数据 - 第59秒数据>
def schedstatDelta(n):
  for folder in folders:
    mpstat_csv_path = os.path.join('.', folder, 'schedstat_data_col_{}.txt'.format(n))
        ...
        for i in range(0,rowNum - 1):
          for j in range(0,colNum):
            ...
            if j==colNum-1:
              ...
              lineBuf = []
        # print(outputs)
        mpstat_csv_path = os.path.join('.', folder, 'schedstat_deltaData_col_{}.txt'.format(n))
...
#       # T=data.T #行列转换
      ...
      plt.ylabel('time ms') #纵坐标名
      plt.title('{}_col_{}'.format(folder,n))
      ...
      sch_fig_path = os.path.join('.', folder, 'schedstat_fig_col_{}.png'.format(n))
      # print(sch_fig_path)
...
schedstatNoColPng(7) #取第N列数据的txt生成png，N从0开始
schedstatNoColPng(8)
schedstatNoColPng(9)